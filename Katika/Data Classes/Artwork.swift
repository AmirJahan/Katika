...
class Artwork: NSObject, NSCoding {
    ...
    func encode(with coder: NSCoder) {
        ...
        if let mySvg = self.svgAsset {
            coder.encode(mySvg, forKey: "svgAsset")

            self.frameString = NSCoder.string(for: svgImgView!.frame)
        }
        if let myTxt = self.txtAsset {
            coder.encode(myTxt, forKey: "txtAsset")

            self.frameString = NSCoder.string(for: textView!.frame)
        }
        if let myImg = self.imgAsset {
            coder.encode(myImg, forKey: "imgAsset")
